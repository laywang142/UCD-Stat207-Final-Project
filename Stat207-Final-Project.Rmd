...
y <- mod2@beta[-c(1:7)]#pulling out the fixed effects from the model
...
In this analysis, it was found that the levels of neural activity in the visual cortex are dependent on an interaction between the left and right contrast levels. However, despite some types of trials being notably easier than others, there were still quite a number of missed trials for said easier combinations. That said, the choice of analytical method for this project was a relatively crude and brute force method. The mean is an overly simplified measure of the neural activity and completely ignores any possible patterns that could correlate with certain decisions being made. Further analysis options for this data set would include attempting to distinguish the neurons by type depending on the responses to certain stimuli. Perhaps a more informative method would be to determine what kinds of firing patterns could be observed among the neurons given certain types of stimuli. As each combination of stimuli shows up at least once in every session, it should be possible to take the average firing rate per neuron per trial type. Then, cluster analysis could be done to determine the possible groupings of the neurons based on the responses to certain stimuli. Finally, models could be fitted for each group to be able to see the differences in neural activity given the contrasts, sessions, and the neuron type. Such an analysis is likely to lead to more definitive and informative conclusions. This could also improve possible prediction models, using the given patterns of how certain neuronal groups fire to determine the outcome of a trial. However, in the interest of time and due to my own shortcomings, I have chosen not to perform such an analysis.     
