...
In the first questions, the focus is on how the stimuli affect the neural activities. In particular, whether or not the stimuli have additive effects on the neural responses. To answer this, a 2-way ANOVA model with mixed effects will be fitted to the entire data set and the variances of a model with only additive effects and a model including an interaction term will be compared. To answer the second question of interest, a predictive model will be built using the first 100 observations of the first session as a testing set and the remaining observations as the training set. The models will be compared using *various criteria* to determine the best concise model.  
...
There are no missing values in this data set and no values for the contrasts in the data set that don't conform to the expected values specified in the description. The times, as discussed in the background section, correspond to the times at which the firing data was collected. Spike data was collected 39 times over the 0.4 second interval. Thus, in the time section, there are 39 times listed per each trial. Another general observation is that the each session happens on a different day. For example, session 1 (Cori) happens on December 14, 2016 and session 2 (also Cori) happens on December 17, 2016. The sessions for Frossman (sessions 4 and 5) are also on different dates. However, because the mice were trained on the protocol beforehand and the experiments were designed to maintain high reward rates, it can be argued that the mice aren't expected to be learning or become unwilling to participate over a few days. Additionally, any mice that developed extreme health issues were excluded from the analysis so it is safe to assume that there are no major physiological changes in these mice that could affect the results between the session dates. 
...
```{r echo=FALSE, fig.cap= "Descriptive Plots", fig.topcaption = TRUE, fig.width=9, message=FALSE, warning=FALSE, out.height='35%'}
#graph showing the distributions of all the rates 

a <- ggplot(dat, aes(x = fire_rate, y = ..count..)) + 
...
In the above interaction plot and main effect plots, there is an indication that there is some sort of interaction between the left and right stimuli. This is observed in the numerous intersections in the interaction plot as well as the grouping of the curves when the left contrast is {1}. Differences in the effect on the firing rate due to the right contrast indicates that an interaction term may be an appropriate addition to the final model.  
...
##### **Figure 3: Average Spks in Time Bin Per Trial** {.tabset .tabset-fade .tabset-pills}  
...
$$Y_{ijk} = \mu + \alpha_{i} + n_{k(i)} + \beta_{i} + (\alpha \beta)_{ij} + \epsilon_{ij}$$ 
...
Assuming the neurological activities correlate with the choosing to perform an action, it should be possible to predict the outcome of the trial given the neurological activity and the levels of the stimuli presented.  
